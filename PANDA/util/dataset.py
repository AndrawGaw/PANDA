...
def make_dataset(split=0, data_root=None, data_list=None, train_class=None):
    ...
    for l_idx in tqdm(range(len(list_read))):
        ...
        gt_name = gt_name.replace('jpg', 'png')
                # ��ӡ·���Լ��
        # print(f"Image: {image_name}, GT: {gt_name}")
        image_class = line_split[1]
        item = (image_name, gt_name, image_class)
        img_gt_class_list.append(item)

        # if int(image_class) not in class_img_gt_dict:
        #     class_img_gt_dict[int(image_class)] = []  # ȷ���ֵ����и���ļ�
...
class SemData(Dataset):
    def __init__(self, split=0, shot=1, data_root=None, data_list=None, transform=None, mode='train'):
        ...
        self.class_list = list(range(1, 19))
        if self.split == 2:
            self.train_class = list(range(1, 13))
            self.val_class = list(range(13, 19))
        elif self.split == 1:
            self.train_class = list(range(1, 7)) + list(range(13, 19))
            self.val_class = list(range(7, 13))
        elif self.split == 0:
            self.train_class = list(range(7, 19))
            self.val_class = list(range(1, 7))
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            return query_rgb, query_mask, s_x, s_y, subcls_list, raw_label, query_img  # ���� query_class
