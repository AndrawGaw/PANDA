...
#         super().__init__()
#         # 2048 (q16) + 1024 (q8) = 3072
#         self.up1 = self.Up(2048 + 1024, 1024, bilinear)

#         # up2: 1024 (上一步输出) + 512 (q4) = 1536
#         self.up2 = self.Up(1024 + 512, 512, bilinear)
#         # 最终的卷积层，将通道数从256变为num_classes
#         self.out_conv = nn.Conv2d(512, num_classes, kernel_size=1)

#     def forward(self, q16, q8, q4):
#         # 上采样并融合 q16 和 q8，输出尺寸 [10, 512, 13, 13]
#         x = self.up1(q16, q8)  
#         # 上采样并融合上一步的输出和 q4，输出尺寸 [10, 256, 25, 25]
#         x = self.up2(x, q4)    
#         # 输出最终的分割结果
#         x = self.out_conv(x)   
#         return x
class UNetSegHead(nn.Module):
    def __init__(self, num_classes=2, bilinear=True):
        super().__init__()
        # q16 引导生成 channel attention，施加到 q8 上
        self.q16_to_q8_attention = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),         # [B, 2048, 1, 1]
            nn.Conv2d(2048, 512, kernel_size=1),  # 压缩维度
            nn.ReLU(inplace=True),
            nn.Conv2d(512, 1024, kernel_size=1),  # 匹配 q8 的通道数
            nn.Sigmoid()
        )
        self.q16_to_q8_attention_weight = nn.Parameter(torch.ones(1))
        # up1: 1024 (增强后的 q8) + 512 (q4) = 1536
        self.up1 = self.Up(1024 + 512, 512, bilinear)
        self.out_conv = nn.Conv2d(512, num_classes, kernel_size=1)

    def forward(self, q16, q8, q4):
        # 获取注意力权重
        attn = self.q16_to_q8_attention(q16)  # [B, 1024, 1, 1]
        q8 = q8+q8 * attn * self.q16_to_q8_attention_weight                        # 通道加权增强
...
class UNetStyleSegHead(nn.Module):
    def __init__(self, num_classes=2, bilinear=True):
        super().__init__()
        ...
        # 上采样与卷积融合模块
        self.up1 = self.Up(2048 + 1024, 1024, bilinear=bilinear)  # q16 + q8
        self.up2 = self.Up(1024 + 512, 512, bilinear=bilinear)    # 上一步输出 + q4
    ...
    def forward(self, q16, q8, q4):
        ...
        q8_fused = self.up1(q16, q8) + q8  # 残差回注
        q4_fused = self.up2(q8_fused, q4) + q4  # 再加残差

        # 最后通道注意力 + 输出
        attn = self.channel_attn(q4_fused)
        x = q4_fused * attn
